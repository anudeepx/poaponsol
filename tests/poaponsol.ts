...
describe("POAPonSOL — Anchor Tests", () => {
  ...
  /**
   * Fetches a Metaplex Core asset and parses it.
   */
  ...
  /**
   * Derives the OrganizerProfile PDA.
   */
   ...
   * NOTE: Based on `close_event.rs` and the provided test, this uses `eventName`.
   * This assumes the provided `create_event.rs` (using `event_count`) is out of sync.
   */
  const getEventPda = (
  ...
  /**
   * Derives the CollectionAuthority PDA.
   */
  ...
  describe("create_event", () => {
    it("Successfully creates an event, profile, and collection", async () => {
      ...
      // 2. Validate OrganizerProfile Account
      ...
      // 3. Validate CollectionAuthority Account
      ...
      // 4. Validate Metaplex Core Collection
  ...
  describe("mint_badge (Happy Path)", () => {
    it("Allows an attendee to mint a badge", async () => {
      ...
      // 1. Validate ClaimRecord Account
      ...
      // 2. Validate Metaplex Core Badge Asset
    ...
    it("Prevents a double claim from the same attendee", async () => {
      ...
      await assert.rejects(
        program.methods
          .mintBadge()
          .accounts({
            claimer: attendee1.publicKey, // Same attendee
            badgeMint: newBadgeMint.publicKey,
            event: eventPda,
            collection: collectionMint.publicKey,
            collectionAuthority: collectionAuthorityPda,
            claimRecord: claimRecordPda1, // Same claim record PDA
            coreProgram: MPL_CORE_PROGRAM_ID,
            systemProgram: SystemProgram.programId,
          } as any)
        ...
        (err: anchor.AnchorError) => {
          // The `init` constraint fails because the account (ClaimRecord) already exists.
          // This is the expected behavior for preventing double claims.
          assert.ok(
      ...
      await assert.rejects(
        program.methods
          ...
          .accounts({
            ...
            event: eventPda,
            collection: fakeCollection.publicKey, // Not the real collection
        ...
        (err: anchor.AnchorError) => {
          ...
          // Checks for constraint: collection.owner == &CORE_PROGRAM_ID
          // or constraint: !collection.data_is_empty()
          // or error: PoapError::InvalidCollection
          assert.ok(
    ...
    it("Prevents a malicious actor from closing the event", async () => {
      await assert.rejects(
        program.methods
          .closeEvent()
          .accounts({
            organizer: maliciousActor.publicKey, // Wrong signer
            event: eventPda,
          })
          .signers([maliciousActor])
          .rpc(),
        (err: anchor.AnchorError) => {
          // Checks for `has_one = organizer @ PoapError::NotAuthorized`
          assert.ok(
    ...
    it("Prevents closing an event that is already closed", async () => {
      await assert.rejects(
        program.methods
          .closeEvent()
          .accounts({
            organizer: organizer.publicKey, // Correct organizer
            event: eventPda, // Already closed event
          })
          .rpc(),
        (err: anchor.AnchorError) => {
          // Checks for `require!(event.is_active, PoapError::EventAlreadyClosed)`
          assert.ok(
    ...
    it("Prevents badge minting after event is closed", async () => {
      await assert.rejects(
        program.methods
          .mintBadge()
          .accounts({
            claimer: attendee2.publicKey, // New attendee
            badgeMint: badgeMint2.publicKey,
            event: eventPda, // Closed event
            collection: collectionMint.publicKey,
            collectionAuthority: collectionAuthorityPda,
            claimRecord: claimRecordPda2, // New claim record
            coreProgram: MPL_CORE_PROGRAM_ID,
            systemProgram: SystemProgram.programId,
          } as any)
          ...
          .rpc(),
        (err: anchor.AnchorError) => {
          // Checks for `constraint = event.is_active @ PoapError::EventNotActive`
          assert.ok(
